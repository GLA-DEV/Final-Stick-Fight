...
musique= PlaySound("Fort Boyard Game.wav", SND_FILENAME | SND_ASYNC)
import credit
def clavier(event):
    global total
    touche = event.keysym
...
def regle():
    ...
    Label(reglee, text="-Vous avez le droit de prendre autant de baton qu'il y en aÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€š  sur la ligne !\n").pack()
...
def on_closing():
    if askyesno("Quitter","Etes vous sur de vouloir quitter ?"):
        if askyesno("Quitter","Vraiment sur ?"):
            if askyesno("Quitter","Genre VRAIMENT SUR ?"):
                if askyesno("Quitter","Bon comme tu veux "):
                    if askyesno("Quitter","Toujours sur ?"):
                        if askyesno("Quitter","Tu peux toujours changer d'avis"):
                            if askokcancel("Quitter","Je vais pas te laisser faire"):
                                if askokcancel("Quitter","on a fait un boutton quitter pour rien ?! "):
                                    if askokcancel("Quitter","Tant pis"):
                                        master.destroy()
                                        musique= PlaySound(None, SND_FILENAME | SND_ASYNC)
master.protocol("WM_DELETE_WINDOW", on_closing)
...
menu2.add_command(label="arreter", command=stopM)
menu2.add_command(label="commencer", command=dem)
